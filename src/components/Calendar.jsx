...
export default function Calendar({ selected, onSelect, bookings = [], blockedDates = [], maxMonthsAhead = 2 }) {
  ...
  const limitTime = new Date(maxLimitDate.getFullYear(), maxLimitDate.getMonth(), 1).getTime();
  const disableNext = currentCursorTime >= limitTime;

  const todayDate = new Date();
  const todayTime = new Date(todayDate.getFullYear(), todayDate.getMonth(), 1).getTime();
  const disablePrev = currentCursorTime <= todayTime;
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const tooFar = ymd > maxLimitStr;
    const hasBooking = bookings.some(b => b.data === ymd && b.status !== 'cancelled');
    cells.push({ day: d, ymd, past, blocked: blocked || tooFar, fullDay: hasBooking, isToday: ymd === today });
  }
  ...
  return (
    <div className="cal">
      <div className="cal-head">
        <button className="cal-nav" onClick={() => setCursor(new Date(y, m - 1, 1))} disabled={disablePrev}>‹</button>
        <div className="cal-title">{MONTHS[m]} de {y}</div>
        <button className="cal-nav" onClick={() => setCursor(new Date(y, m + 1, 1))} disabled={disableNext}>›</button>
      </div>
